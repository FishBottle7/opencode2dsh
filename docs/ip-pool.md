...
### 4.1 探活的并发与模型范围（定案，2026-09-04）

三条硬约束，实现时不要「优化」掉：

1. **同一出口（IP）串行**：同一出口的探测任务（哪怕测不同模型）排队执行，上一发返回（或超时）才发下一发。同一个 IP 的配额是共享桶，并发打它就是自己挤兑自己。
2. **不同出口并发，但有全局上限**：不同出口 IP 的配额各自独立，可以并发；`maxConcurrentProbes`（默认 **3**）封顶——保护的对象有三层：上游（匿名通道看到的总请求率）、被探测的公共代理（连接压力）、本机带宽。可调（1 = 退化全串行，用户显式改）。**准入探测（§4.5）同样受这个上限约束**——免费源一轮能拉出几百个候选，准入不设闸就是扫描行为。
...
实现要点：Prober 是一个**两级调度队列**——外层 `maxConcurrentProbes` 个工位，内层 per-exit FIFO（出口在飞探测数 ≤ 1，由 §3.2 `inflight` 锁位表达）。探测顺序：同一出口的多个模型连续探完（省得反复抢同一出口的锁），不同出口交错推进。设置页点「批量探活」看到的是队列进度（x/y），点单出口「探测」是该出口的一次插队（仍遵守串行与全局上限）。
...
| # | 步骤 | 拿什么 | 弃用条件 |
| --- | --- | --- | --- |
| 1 | 连通 + 延迟 | 经候选代理 GET 一个轻量 HTTP 端点（http://ip-api.com 一石二鸟，GoProxy validateURL 同思路） | 连不上 / 超过 maxResponseMs（默认 3000，critical 态放宽到 6000） |
| 2 | 出口 IP + 地理 | 同一响应体（ip-api.com 返回 query=IP + countryCode/city） | 拿不到 exitIP（§3.1 必填）；地理黑名单命中（blockedCountries，默认 `['CN']`——直连已可达，无需绕；GoProxy BLOCKED_COUNTRIES 同默认） |
| 3 | HTTPS 隧道 | 经代理 CONNECT 一个 HTTPS 目标（轮换目标列表，GoProxy httpsTestTargets 思路；我们直接用 `{zen}/v1/models` 一次轻量 GET——上游即最终目标，比探 google 更真实） | 隧道建立失败 |
| 4 | 匿名通道冒烟 | §4.2 的 1-token chat 探测（probeModels 首个） | 4xx/5xx（进池前就 dead 的不要） |

第 4 步把「GoProxy 的准入验证」与「我们的匿名通道探活」合并成一条链——GoProxy 验证到「HTTPS 隧道通」为止，我们多走一步直接验证最终目标，免费候选里对匿名通道 4xx 的在门口就挡掉，不占池位。

- **准入并发**：与周期探活共享 `maxConcurrentProbes` 工位与两级调度（§4.1；同一候选地址在飞 ≤ 1）。一轮几百个候选按批流入：状态机（§3.5）按「目标容量 - 现可用数」计算本轮准入额度，够了就提前终止（GoProxy NeedsFetchQuick 同义）。
- **manual/subscription/goproxy 来源**：手填、订阅（清单已含节点详情）与 GoProxy 清单行（它已验证过出口 IP）跳过 1-3 步，只跑第 4 步冒烟；失败不禁用 subscription/goproxy 来源节点（可能是暂时的），只标 suspect。**pinned 节点例外**：准入 1-3 步失败也收（exitIP 留空、路由键退化为 address，§3.1），只在第 4 步冒烟失败时警告——专线场景用户判断优先。
...
| maxConcurrentProbes | **3** | 全局并发上限（跨出口，探活+准入共享工位）；设 1 = 全串行 |
